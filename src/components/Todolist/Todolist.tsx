  ...
  return (
    <div className={style.todolist}>
      <div className={style.todolist_header}>
        {error ? (
          <h3 className={style.error_message}>{error}</h3>
        ) : (
          <h3>{props.title}</h3>
        )}
        <button
      ...
      <div className={style.todolist_filter}>
        <button
          className={`${style.btn} ${style.btn_filter} ${
            props.filter === "all" ? style.active_filter : ""
          }`}
        ...
        <button
          className={`${style.btn} ${style.btn_filter} ${
            props.filter === "active" ? style.active_filter : ""
          }`}
        ...
        <button
          className={`${style.btn} ${style.btn_filter} ${
            props.filter === "completed" ? style.active_filter : ""
          }`}
            ...
            <li className={`${style.task_item}`}>
              <label htmlFor={keyForLabel}>
                <input
                  ...
                  onChange={onChangeHandler}
                />
                <span className={`${t.isDone ? style.task_isDone : ""}`}>
                  {t.title}
                </span>
              </label>
              <button
